...
struct AdjListNode* newAdjListNode(int nodeIndex, int weight) {
...
struct Graph* createGraph(LL V) {
    ...
    assert((V * sizeof(struct AdjList)) > 0);
...
int main() {
    ...
    while (T--) {
        ...
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < F; ++j) {
                if (i != N-1) {
                    if (j != F-1) addEdge(graph, (i*F) + j, (i*F) + j + 1, 1);
                    if (j==0) addEdge(graph, (i*F) + j, ((i+1)*F) + j, 1);
                }
                if (i == N-1) {
                    if (j != F-1) addEdge(graph, (i*F) + j, (i*F) + j + 1, 1);
                    if (j==0) addEdge(graph, (i*F) + M, M, 1);
                }
            }
        }
        for (int i = 0; i < M; ++i) {
            ...
            addEdge(graph, ((tx-1)*F)+(fx-1), ((ty-1)*F)+(fy-1), w);
        }
        int Q;
        scan(&Q);
        for (int i = 0; i < Q; ++i) {
            ...
            dijkstra(graph, ((qtx-1)*F)+(qfx-1), ((qty-1) *F)+ (qfy-1));
            //cout<<i<<endl;
        }
    } 
    return 0;
